...
def DAC_signal_properties(DAC_v):
    """Finds the properties of the DAC signal data
    output: signal_frames, signal_offset, signal_amplitude, signal_shape
    """
    ff = np.fft.rfft(DAC_v)
    fr = len(DAC_v)/(np.max([np.argmax(np.abs(ff[1:])),1])+1)
    start = np.argmax(DAC_v[:int(fr)])
    argmaxlist = [start]
    while(argmaxlist[-1]+fr*1.25 < len(DAC_v)):
        argmaxlist.append(int(argmaxlist[-1]+fr*0.75)+np.argmax(DAC_v[int(argmaxlist[-1]+fr*0.75):int(argmaxlist[-1]+fr*1.25)]))
    signal_shape = 'triangle'
    signal_frames = np.mean(np.diff(argmaxlist))
    signal_offset = np.mean(argmaxlist%signal_frames)
    signal_amplitude = np.mean(DAC_v[argmaxlist])
    return signal_frames, signal_offset, signal_amplitude, signal_shape
...
def movie_signal_offset(movie, signal):
    """Returns the difference in maximum peaks from the movie and signal
    output: moviesignaloffset
    """
    movieframe = np.argmax(movie[:30])
    signalframe = np.argmax(signal[:])
...
def find_FWHM_for_particle(mean_img, x_found, y_found, x_search=30, y_search=15):
